...
void renderScene(GLFWwindow* window)
{
	...
	for (auto& asteroid : asteroids) {
		glm::mat4 modelMatrix = glm::translate(asteroid.startPosition) * glm::scale(asteroid.modelScale);
		drawObjectTexture(sphereContext, modelMatrix, texture::asteroid, FALSE);
	}
	...
	// This rotation matrix will rotate the spaceship by 90 degrees around the Y-axis.
	...
	// Apply the rotation to the spaceship's current transformation matrix.
	...
	// Now draw the spaceship with the updated model matrix.
